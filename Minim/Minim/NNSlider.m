...
- (void)setDefaults {
    ...
    self.isSegmented = NO;
    self.segments = 5;
    self.snapsToSegments = YES;
    self.segmentLineWidth = 1.5;
    self.segmentLineSize = 3;
    self.segmentLineOffset = 0;
...
- (void)makeLabel {
    ...
    if (self.isString == YES) {
        label.text = [NSString stringWithFormat:@"%.2f | %.2f",self.value,self.altValue];
    } else {
        label.text = [NSString stringWithFormat:@"%.2f",self.value];
    }
    ...
    if (lineLayer == nil)
    {
        ...
        if (self.isSegmented == YES) {
            for (int i = 1; i < self.segments; i++) {
                ...
                segments.lineWidth = self.segmentLineWidth;
...
- (UIBezierPath *)segmentPathFor:(int)loop
{
    ...
    if (isVertical == NO) {
        ...
        float originY = (self.bounds.origin.y+(self.bounds.size.height/2))-self.segmentLineWidth+self.segmentLineOffset;
        float originX = self.bounds.origin.x+deltaX;
        [path moveToPoint: CGPointMake(originX, originY)];
        [path addLineToPoint:CGPointMake(originX, (self.bounds.size.height/2)+self.segmentLineSize+self.segmentLineOffset)];
        [path stroke];
    } else {
        float endY = self.bounds.size.height;
        float deltaY = (endY/self.segments)*loop;
        float originY = self.bounds.origin.y+deltaY;
        float originX = (self.bounds.origin.x+(self.bounds.size.width/2))-self.segmentLineSize+self.segmentLineOffset;
        //float originX = (self.bounds.origin.x+self.bounds.size.width/2)-(self.bounds.size.width*.2);
        [path moveToPoint: CGPointMake(originX, originY)];
        [path addLineToPoint:CGPointMake((self.bounds.size.width/2)+self.segmentLineSize+self.segmentLineOffset, originY)];
        [path stroke];
    }
